...
label to re-run on a later push). It needs the `OPENAI_API_KEY` repository
secret. Codex loads this file on its own, so the review is judged against the
"Code review" section below; nothing agent-specific lives in the workflow.
